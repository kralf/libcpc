...
HIDE_SYMBOL char * ini_get_key_value(const char *section, const char *skey, const char *file, char *dst, size_t size)
{
...
int HIDE_SYMBOL ini_get_section_list(const char *file, char **list, int maxsections)
{
...
int main(int argc, char **argv)
{
	...
	for(i = i -1; i >= 0; i--) {
		printf("%s\n", list[i]);
		printf("\tType = %s\n", ini_get_key_value(list[i], "InterfaceType", argv[1], buf, sizeof(buf)));
		printf("\tIP = %s\n", ini_get_key_value(list[i], "SlotNr", argv[1], buf, sizeof(buf)));
		free(list[i]);
	}
